<template>
    <div class="grid px-1 w-full">
        ...
        <div class=" col-12 flex flex-wrap gap-2 align-items-center">
            <Calendar v-model="current_month" view="month" dateFormat="mm/yy" @date-select="changeCalendar"
                :loading="loading" :manual-input="false" style="width: 200px;" class="p-inputtext-sm" />
                ...
                <div class="tabel-container">
                    <table class="tabel-box" v-if="!loading">
                        <tr class="tabel-row tabel-row-header">
                            ...
                            <td class="tabel-col" v-for="(month_day, index) in Tabel_days" :key="index"
                        ...
                        <tr class="tabel-row" v-for="(cadry, cadry_index) in Tabel_cadry" :key="cadry">


                            <td class="tabel-col-header" @click="select_row(cadry_index)"> {{ `(${cadry.number})- ` +
                                cadry.fullname }}</td>
                    ...
                    <div class="grid w-full relative">
                        <div class="col-12  flex justify-content-center align-items-center relative p-4">
                            <Paginator :template="{
                            ...
                            }" v-model:first="current_page" v-model:rows="params.per_page" :totalRecords="total_item"
                                v-show="total_item > 10" class="py-0 text-base"
                    ...
                    <ProgressBarLoader v-if="loading" :min_height="'400px'"></ProgressBarLoader>
                </div>
            </div>
        ...
        <div class="xl:col-8 lg:col-7 col-12 flex flex-wrap gap-2 align-content-center">
            <Dropdown id="tabel_types" style="min-width:200px; width:500px" v-model="type_val" :options="category_List"
                :loading="loading" optionLabel="fullname" :filter="true" placeholder="Tanlang" @change="changeType">
            ...
            <InputNumber :disabled="loading" v-model="work_time" inputId="minmax-buttons" mode="decimal" showButtons
                :min="0" :max="100" placeholder="Ish vaqti" />
        ...
        <div class="xl:col-4 lg:col-5 col-12 mb-6 xl:mb-0 lg:mb-0 md:mb-0 flex justify-content-end align-items-center">
            <Button icon="pi pi-download" label="" class="p-button-success text-sm font-normal uppercase mr-2"
                @click="dowload_exel()" :loading="download_loading"></Button>
            <Button icon="pi pi-save" label="Saqlash" class="p-button-primary text-sm font-normal uppercase"
                @click="save_tabel()" :loading="save_loading"></Button>
        </div>
        <div class="col-12">
...
export default {
    ...
    methods: {
        ...mapActions(["set_tabel_days", "set_category_list", "set_cadry_list", "set_category_val", "set_tabel"]),
        ...
        async select_day(cadry_id, day, cadry_index, day1_index) {
            if (this.is_delete) {
                ...
                this.Tabel_cadry[cadry_index].days[day1_index].category_id = this.current_work_type;
                this.Tabel_cadry[cadry_index].days[day1_index].work_time = this.current_work_time;
            }
            return true
        ...
        select_row(cadry_index) {
            for (let item = 0; item < this.Tabel_cadry[cadry_index].days.length; item++) {

                if (this.is_delete) {
                    this.Tabel_cadry[cadry_index].days[item].category_id = null;
                    this.Tabel_cadry[cadry_index].days[item].work_time = null;
                } else {
                    if (this.Tabel_cadry[cadry_index].days[item].category_id == null && this.Tabel_cadry[cadry_index].days[item].work_time == null) {
                        this.Tabel_cadry[cadry_index].days[item].category_id = this.current_work_type;
                        this.Tabel_cadry[cadry_index].days[item].work_time = this.current_work_time;
                    }
        ...
        select_col(day_index) {
            this.Tabel_cadry.forEach((cadry => {
                ...
                if (this.is_delete) {
                    ...
                    if (cadry.days[day_index].category_id == null && cadry.days[day_index].work_time == null) {
        ...
        auto_select() {
            this.all_cadry_list.forEach((cadry) => {
                cadry.days.forEach((day, index) => {
                    if ((day.category_id == null && day.work_time == null) && this.current_work_type != 3) {
        ...
        search_name(search_text) {
            if (search_text) {
                ...
                let number = 1;
                this.all_cadry_list.forEach((cadry) => {
                    if (cadry.fullname.toLowerCase().search(search_text.toLowerCase()) !== -1) {
                        cadry.number = number++
        ...
        dowload_exel() {
            ...
            TabelService.dowload_Tabels({
                ...
                fileLink.setAttribute('download', `Tabel(${this.select_date.month > 10 ? this.select_date.month : '0' + this.select_date.month}-${this.select_date.year}).xlsx`);
